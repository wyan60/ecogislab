...
class mh_magazine_posts_list extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_magazine_posts_list', esc_html_x('MH Posts List', 'widget name', 'mh-magazine'),
			array(
				'classname' => 'mh_magazine_posts_list',
				'description' => esc_html__('MH Posts List widget to display a list of posts with thumbnails.', 'mh-magazine'),
				'customize_selective_refresh' => true
			)
		);
	}
	function widget($args, $instance) {
		$defaults = array('title' => '', 'link' => '', 'category' => 0, 'cats' => '', 'tags' => '', 'postcount' => 3, 'offset' => 0, 'order' => 'date', 'category_label' => 0, 'sticky' => 1);
        ...
        echo $args['before_widget'];
			if ($widget_posts->have_posts()) :
				...
				if ($instance['category_label'] == 0) {
					$caption_class = ' mh-posts-list-hide-caption';
				} else {
					$caption_class = '';
				}
				echo '<div class="mh-posts-list-widget' . esc_attr($caption_class) . '">' . "\n";
