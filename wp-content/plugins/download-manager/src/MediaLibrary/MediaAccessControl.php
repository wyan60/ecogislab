...
class MediaAccessControl {
	...
	function makeMediaPass() {
		...
		wp_send_json( array(
			'success' => false,
			'error'   => __( "<b>Error:</b> Wrong Password! Try Again.", "download-manager" )
		) );
	}

	function mediaShortcode( $params ) {
		if ( ! isset( $params['id'] ) || (int) $params['id'] < 1 ) {
			return __( "Missing ID!", "download-manager" );
		}
		$media           = get_post( $params['id'] );
		$media->path     = str_replace( home_url( '/' ), ABSPATH . '/', $media->guid );
		$media->filesize = wpdm_file_size( $media->path );
		$picon           = ! substr_count( $media->post_mime_type, 'image' ) ? wp_get_attachment_image( $media->ID, 'thumbnail', true ) : UI::img( wpdm_dynamic_thumb( $media->path, [
			128,
			128
		], true ), 'Thumb', [ 'width' => 48 ] );
		$media->icon     = $picon;
	...
	function protectMediaLibrary(){
		if(isset($_REQUEST['wpdmmediaid'])){
			...
			$media = get_post($ID);
			if(!$media) Messages::fullPage('404', esc_attr__( 'Media not found!', "download-manager" ));
			...
			if( $private && ( $password || !$user_allowed ) ) {
				...
				Messages::fullPage("Error!", UI::div('Direct access disabled!', 'alert alert-danger', ['style' => 'display:table;margin: 0 auto']));
	...
	function protectMediaLibrary() {
		if ( isset( $_REQUEST['wpdmmediaid'] ) ) {
			...
			$media        = get_post( $ID );
			if ( ! $media ) {
				Messages::fullPage( '404', esc_attr__( 'Media not found!', WPDM_TEXT_DOMAIN ) );
			}
			$media_meta = wp_get_attachment_metadata( $ID );
			//wpdmdd($media);
			...
			if ( ! $media_rel_path && isset( $media['file'] ) ) {
				$media_rel_path = $media['file'];
			}
	...
	function mediaAccessControl() {
        ...
        <div class="panel panel-default"
             id="__protm" <?php if ( $wpdm_media_private ) { ?> style="display: none" <?php } ?>>
            <div class="panel-body">
				<?php _e( 'This file is not protected.', 'download-manager' ) ?>
            </div>
            <div class="panel-footer">
                <button class="btn btn-success btn-block"
                        onclick="jQuery('#__protm').slideUp();jQuery('#__prots').slideDown();"><?php _e( 'Protect this file', 'download-manager' ); ?></button>
            </div>
        </div>

        <div id="__prots"
             class="panel panel-default" <?php if ( ! $wpdm_media_private ) { ?> style="display: none" <?php } ?>>
			<?php if ( $wpdm_media_private ) { ?>
                <div class="panel-body text-danger"><i class="fa fa-lock"></i>
                &mdash; <?php echo __( "This file is protected", "download-manager" ); ?></div><?php } ?>
            <div class="panel-heading"><?php _e( 'Password:', 'download-manager' ); ?></div>
            <div class="panel-body"><input type="text" value="<?php echo $wpdm_media_pass; ?>" id="media_pass"
                                           class="form-control"
                                           style="border-radius: 3px;border: 0;box-shadow: none !important;"
                                           name="media[password]"
                                           placeholder="<?php _e( 'Password', 'download-manager' ); ?>"/></div>
            <div class="panel-heading"
                 style="border-radius: 0;border-top: 1px solid #ddd"><?php _e( 'Allow Access:', 'download-manager' ); ?></div>
            <div class="panel-body" id="acx">
				...
				$selz = '';
				if ( is_array( $wpdm_media_access ) ) {
					$selz = ( in_array( 'public', $wpdm_media_access ) ) ? 'checked=checked' : '';
				}

				?>

                <label><input class="media_access" style="margin: 0" name="media[access][]" type="checkbox"
                              value="public" <?php echo $selz ?>> <?php echo __( "Public", "download-manager" ); ?>
                </label>
				<?php
				global $wp_roles;
				...
				foreach ( $roles as $role => $name ) {
					...
					if ( is_array( $wpdm_media_access ) ) {
						$sel = ( in_array( $role, $wpdm_media_access ) ) ? 'checked=checked' : '';
					} else {
						$sel = '';
					}
					...
					?>
                    <label><input class="media_access" style="margin: 0" name="media[access][]" type="checkbox"
                                  value="<?php echo $role; ?>" <?php echo $sel ?>> <?php echo $name; ?></label>
				<?php } ?>
            ...
            <div>
                <input onfocus="this.select()" type="text" readonly="readonly" class="form-control"
                       style="border: 0;box-shadow: none !important;text-align: center;background: #ffffff;font-family: monospace"
                       value="[wpdm_media id=<?php echo $id; ?>]"/>
            </div>
            <div class="panel-footer">
                <button class="btn btn-block btn-primary btn-sm" id="__makeprivate"
                        data-id="<?php echo $id; ?>"><?php _e( 'Apply restrictions', 'download-manager' ); ?></button>
            </div>
            <div class="panel-footer">
                <button class="btn btn-block btn-danger btn-sm" id="__makepublic"
                        data-id="<?php echo $id; ?>"><?php _e( 'Remove all restrictions', 'download-manager' ); ?></button>
            </div>
        </div>
        ...
        <script>
            ...
            jQuery(function ($) {
                ...
                $('body').on('click', '#__makepublic', function () {
                    ...
                    $.post(ajaxurl, {
                        action: 'make_media_public',
                        mmpnonce: '<?php echo wp_create_nonce( WPDM_PRI_NONCE ); ?>',
                ...
                $('body').on('click', '#__makeprivate', function () {
                    ...
                    $.post(ajaxurl, {
                        action: 'make_media_private',
                        mmpnonce: '<?php echo wp_create_nonce( WPDM_PRI_NONCE ); ?>',
                        mediaid: $(this).data('id'),
                        media_pass: $('#media_pass').val(),
